...
func TestCommitOperation(t *testing.T) {
	...
	finalIndex, _ := logfile.CommitOperation(0, &Transaction{Index: 1, Operation: "add:3", Term: 1})
...
func TestApplyOperation(t *testing.T) {
	...
	finalIndex1, _ := logfile.CommitOperation(0, &Transaction{Index: 1, Operation: "add:3", Term: 1})
	assert.Equal(t, 0, finalIndex1, "final index should be 0 for first commit operation")

	err := logfile.ApplyOperation()
	assert.Equal(t, nil, err, "no error should be present while applying operation 1")

	finalIndex2, _ := logfile.CommitOperation(1, &Transaction{Index: 2, Operation: "add:4", Term: 2})
...
func TestGetLastTransaction(t *testing.T) {
	...
	finalIndex3, _ := logfile.CommitOperation(2, &Transaction{Index: 3, Operation: "add:5", Term: 2})
